...
const FacultyLogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simple validation for faculty email
    if (!email.includes('.com')) {
      toast({
        title: 'Invalid Email',
        description: 'Faculty email must contain .com domain',
        variant: 'destructive',
      });
      setIsLoading(false);
      return;
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="text-center text-sm">
                <p className="text-muted-foreground">
                  Use demo account: faculty@example.com / password123
                </p>
              </div>
            </form>
          </CardContent>
          <CardFooter className="flex flex-col space-y-4">
            <div className="text-center text-sm">
              <Link to="/auth/choice" className="text-primary hover:underline">
                Not faculty? Choose another login option
              </Link>
            </div>
          </CardFooter>
        </Card>
